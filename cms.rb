...
def submit_new_document_form(file)
  file << '.txt' if File.extname(file).empty?
  ...
  if user_not_signed_in
    ...
    if extension == '.md'
      ...
      File.open(file_path, 'a+') do |file|
        file.write("![image](#{params[:image_select]})")
      end
    elsif extension == '.html'
      # add image formatted for HTML
      File.open(file_path, 'a+') do |file|
        file.write("<img src=#{params[:image_select]}>")
      end
    else
...
post '/create' do
  if user_not_signed_in
    redirect_to_homepage
  elsif params[:new_document].strip.empty?
...
post '/upload' do
  if user_not_signed_in
    redirect_to_homepage
  elsif params[:file] && image?(params[:file][:filename])
...
post '/:file/delete' do
  if user_not_signed_in
    ...
    if image?(params[:file])
      File.delete(File.join(image_path, params[:file]))
    else
...
post '/:file/rename' do
  FileUtils.copy(File.join(data_path, params[:file]),
                 File.join(data_path, params[:new_file_name]))
